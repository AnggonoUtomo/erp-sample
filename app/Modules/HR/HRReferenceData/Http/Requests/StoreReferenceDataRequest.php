...
class StoreReferenceDataRequest extends FormRequest
{
    ...
    {
        return $this->user()?->can('create', ReferenceData::class) ?? false;
    }
    ...
    public function rules(): array
    {
        return [
            ...
            'code' => [
                ...
                'max:64',
                'alpha_dash:ascii',
                Rule::unique('hr_reference_data', 'code')->where(fn ($query) => $query->where('category', str((string) $this->input('category'))->lower()->kebab()->toString())),
            ],
