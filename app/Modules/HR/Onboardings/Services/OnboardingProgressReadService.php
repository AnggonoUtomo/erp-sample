...
class OnboardingProgressReadService
{
    ...
    public function detail(Onboarding $onboarding, string $businessDate): array
    {
        ...
        return [
            ...
            'completed_by' => $onboarding->completedBy?->only(['id', 'name']),
            'completed_at' => $onboarding->completed_at?->toIso8601String(),
            'cancelled_by' => $onboarding->cancelledBy?->only(['id', 'name']),
            'cancelled_at' => $onboarding->cancelled_at?->toIso8601String(),
            'cancel_reason' => $onboarding->cancel_reason,
            'progress' => [
                'total' => $total,
