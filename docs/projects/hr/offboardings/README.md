...
`TASK 08 COMPLETE` — Task 01–08 dan draft snapshot checkpoint selesai pada 2026-07-16. Authorized HR dapat mengelola template, membuat draft idempotent dengan ordered task snapshot atomic, membaca detail/progress deterministic, mengaktifkan draft eligible, lalu mengelola assignee serta lifecycle `PENDING -> IN_PROGRESS -> COMPLETED`. Completion evidence menyimpan actor, waktu, dan catatan secara atomic serta dapat dibaca di UI. Mutation task memakai policy, scoped binding, row lock, audit, rollback, dan idempotency guard. Skip/reopen, readiness, cancel, final employment exit, serta integration event downstream tetap deferred.
